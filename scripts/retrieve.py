...
def load_index() -> tuple[list[dict], np.ndarray]:
    ...
    if not chunks_path.exists() or not embeddings_path.exists():
        raise FileNotFoundError(f"Index not found at {INDEX_DIR}. Run: python -m scripts.embed_index")
    chunks = [json.loads(line) for line in chunks_path.read_text().splitlines() if line.strip()]
    embeddings = np.load(str(embeddings_path))
    return chunks, embeddings


def embed_query(query: str, provider: str = "local", model: str | None = None) -> np.ndarray:
    if provider == "openai":
        from openai import OpenAI
        client = OpenAI()
        resp = client.embeddings.create(model=model or "text-embedding-3-small", input=[query])
        return np.array(resp.data[0].embedding, dtype=np.float32)
    from sentence_transformers import SentenceTransformer
    m = SentenceTransformer(model or "all-MiniLM-L6-v2")
    return m.encode([query], normalize_embeddings=True)[0]
...
def retrieve(query: str, top_k: int = 5, provider: str = "local", model: str | None = None) -> list[dict]:
