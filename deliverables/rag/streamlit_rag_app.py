...
Search the entire codebase with natural language.
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**Build index first:**\n```\npython -m scripts.embed_index\n```")

query = st.text_input("Search", placeholder="e.g. how does the invoice template work?")
    ...
    try:
        ...
        if not results:
            ...
            for i, r in enumerate(results, 1):
                with st.expander(f"#{i} `{r['source']}`" + (f" — score {r['score']:.3f}" if show_score else ""), expanded=(i == 1)):
                    st.code(r["text"][:400 if not show_full_text else 9999] + ("..." if not show_full_text and len(r["text"]) > 400 else ""), language="text")
